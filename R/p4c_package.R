...
#'
#' @export
p4cNewProfile <- function(
    track_nm, bait_chrom = gtrack.attr.get(track_nm[1], "Bait_chr"),
    ...
    if (bait_start < scope_5) {
        warning(
            "The bait coordinate is ", bait_start, " but the scope_5 was set to ", scope_5,
    ...
    if (bait_start + scope_3 > chrom_size) {
        warning(
            "The bait coordinate is ", bait_start, " and the chromosome size is ",
    ...
    p4c_obj <- list(
        track_nm = paste(track_nm, collapse = " "), bait = list(
            chrom = bait_chrom,
...
print.p4cProfile <- function(p4c_obj) {
    ...
    cat("Current scope: \n")
    cat(
        "scope_5:", p4c_obj$scope["scope_5"], " scope_3:", p4c_obj$scope["scope_3"],
        "\n"
    )
...
summary.p4cProfile <- function(p4c_obj) {
    ...
    vec <- c(
        `Covered fends` = round(covered_fends), `Total contacts` = round(total_contacts),
...
#'
#' @export
plot.p4cProfile <- function(
    p4c_obj1, p4c_obj2, trend_scale = "adaptive", png_fn = NULL,
    ...) {
    if (missing(p4c_obj2)) {
        plotSingleProf(
            p4c_obj = p4c_obj1, trend_scale = trend_scale, png_fn = png_fn,
            ...
        )
    }

    if (!missing(p4c_obj1) & !missing(p4c_obj2)) {
        plotCompProf(p4c_obj1, p4c_obj2,
            trend_scale = trend_scale, png_fn = png_fn,
            ...
        )
    }
}
...
#' @export
plotSingleProf <- function(
    p4c_obj, trend_scale, png_fn, plot.colorbar, add.func,
    xlim, ylim, trend_only, main, sd, ...) {
    UseMethod("plotSingleProf")
}

#' @export
plotSingleProf.p4cProfile <- function(
    p4c_obj, png_fn = NULL, trend_scale = "adaptive",
    ...
    if (is.numeric(trend_scale)) {
        ...
        cur_desc <- sprintf(
            "%s _ UMI-4C track smoothed values in scope (fixed trend scale: %i fragments)",
        ...
        cur_desc <- sprintf(
            "%s _ UMI-4C track smoothed values in scope (min coverage - %i molecules)",
    ...
    if (!missing(filename1)) {
        header_line1 <- sprintf("browser position %s:%i-%i", paste0("chr", gsub(
            "chr",
            "", p4c_obj$bait$chrom
        )), round(coords[1]), round(tail(coords, 1)))
        header_line2 <- sprintf(
        ...
        bed_df <- data.frame(
            chrom = paste0("chr", gsub("chr", "", p4c_obj$bait$chrom)),
    ...
    if (!is.null(png_fn)) {
        ...
        png(png_fn,
            width = p4c_obj$figure_params$png_w, height = p4c_obj$figure_params$png_h,
    ...
    # plot trend
    plot(coords, pmin(trend, ylim[2]),
        type = "l", lwd = 2, col = "black", axes = F,
    ...
    if (is.numeric(trend_scale)) {
        ...
        polygon(c(coords, rev(coords)), c(trend_nna - sd * sqrt(trend_nna / (trend_scale *
            2)), rev(trend_nna + sd * sqrt(trend_nna / (trend_scale * 2)))),
        col = "grey",
        border = 0
        )
    }
    if (trend_scale == "adaptive") {
        ...
        polygon(c(coords, rev(coords)), c(trend_nna - sd * sqrt(trend_nna / (scales *
            2)), rev(trend_nna + sd * sqrt(trend_nna / (scales * 2)))),
        col = "grey",
        border = 0
        )
    }
    ...
    message(
        "Range of values in dgram: from ", min(dgram_orig[, n:3], na.rm = T),
        " to ", max(dgram_orig[, n:3], na.rm = T)
    )
    message(
        "Transformed to log2(fraction of max): from ", min(dgram[, n:3], na.rm = T),
        " to ", max(dgram[, n:3], na.rm = T)
    )
    shades <- colorRampPalette(p4c_obj$graphics_params$shades)(1000)

    image(
        x = coords, z = pmin(dgram[, n:3], p4c_obj$graphics_params$zlim[2]), col = shades,
    ...
    axis(1, round(seq(horiz5, horiz3, l = 9) / 1e+06, 2) * 1e+06, labels = round(seq(horiz5,
        horiz3,
        l = 9
    ) / 1e+06, 2), cex.lab = 2, cex.axis = 1.4)

    scales_l <- length(p4c_obj$dgram_params$dgram_scales)
    axis(2, c(0, 0.5),
        labels = c(
            p4c_obj$dgram_params$dgram_scales[scales_l],
    ...
    sapply(missing_re_idx, function(i) {
        polygon(
            c(
                coords_fends[i] - 10000, coords_fends[i],
                coords_fends[i + 1], coords_fends[i + 1] + 10000
            ), c(0, 1, 1, 0),
            col = "white",
            border = NA
        )
    ...
    if (plot.colorbar) {
        ...
        par(mgp = c(2, 0.8, 0))
        .image.scale(
            zlim = p4c_obj$graphics_params$zlim, col = colorRampPalette(p4c_obj$graphics_params$shades)(100),
        ...
        axis(2,
            at = expand_axis_range, labels = sprintf("%.02f%%", 100 * (2^expand_axis_range)),
            las = 2
        )
...
#' @rdname plot.p4cProfile
#' @export
plotCompProf <- function(
    p4c_obj1, ref_p4c_obj, trend_scale, png_fn, col, min_win_cov,
    xlim, zlim, legend.text, ylim, dgram.method, main, sd, filename1, filename2, trend_only) {
    UseMethod("plotCompProf")
}
...
plotCompProf.p4cProfile <- function(p4c_obj1, ref_p4c_obj, trend_scale = "adaptive",
    ...
    if (trend_scale != "adaptive") {
        message(
            "Only the trend scale adaptive is currently supported in plot",
    ...
    if (!missing(filename1)) {
        ...
        cur_desc <- sprintf(
            "%s _ UMI-4C track smoothed values in scope (min coverage - %i molecules) when norm to %s",
        ...
        header_line1 <- sprintf("browser position %s:%i-%i", paste0("chr", gsub(
            "chr",
            "", p4c_obj1$bait$chrom
        )), coords[1], tail(coords, 1))
        header_line2 <- sprintf(
            "track type=bedGraph name=\"%s\" description=\"%s\" color=%s",
        ...
        bed_df <- data.frame(
            chrom = paste0("chr", gsub("chr", "", p4c_obj1$bait$chrom)),
            start = round(coords), end = round(coords) + 1,
            value = trend1
        )
    ...
    if (!is.null(png_fn)) {
        ...
        png(png_fn,
            width = p4c_obj1$figure_params$png_w, height = p4c_obj1$figure_params$png_h,
    ...
    plot(coords, trend1,
        lwd = 2, type = "l", ylim = ylim, xlab = "", ylab = "",
    ...
    points(ref_p4c_obj$dgram[, 1], pmin(ref_p4c_obj$dgram[, 2], ylim[2]),
        col = col[2],
        pch = 16, cex = 0.7
    )
    ...
    scales[is.na(scales)] <- 1
    polygon(c(coords, rev(coords)), c(
        trend_nna - sd * sqrt(trend_nna / (scales * 2)),
        rev(trend_nna + sd * sqrt(trend_nna / (scales * 2)))
    ),
    col = "grey", border = "dark grey",
    density = 55
    )
    ...
    leg_text <- paste0(c(paste0(p4c_obj1$track_nm, " N=", round(sum(p4c_obj1$dgram[
        ,
        2
    ], na.rm = T))), paste0(ref_p4c_obj$track_nm, " N=", round(sum(ref_p4c_obj$dgram[
        ,
        2
    ], na.rm = T)))))
    ...
    image(
        x = coords, z = as.matrix(scales), axes = F, col = colorRampPalette(s_cols)(100),
        xlim = xlim
    )
    ...
    if (dgram.method == "delta") {
        ...
        image(x = coords, z = dlt_dgram[, n:2], axes = F, col = colorRampPalette(c(
            col[2],
            "white", col[1]
        ))(1000), zlim = zlim, bty = "o", xlim = xlim, )
        ...
        image(
            x = coords, z = pmin(dgram[, n:3], p4c_obj1$graphics_params$zlim[2]),
            col = shades, xlab = sprintf("chrom %s coordinate (Mb)", gsub(
                "chr",
                "", p4c_obj1$bait$chrom
            )), zlim = p4c_obj1$graphics_params$zlim,
...
.p4cReportIntervalData <- function(
    p4c_obj, ref_p4c_obj, start, end, min_win_cov = 30,
    rawdata = FALSE) {
    # normalize p4c_obj1 to p4c_obj2
    ...
    if (!rawdata) {
        ...
        slice_df <- data.frame(p4c_obj$dgram.norm$dgram[scope_idx, 1:2], ref_p4c_obj$dgram[
            scope_idx,
            2
        ], rep(NA, length(scope_idx)))
    }
...
#'
#' @export
p4cIntervalsMean <- function(
    p4c_obj, ref_p4c_obj, start, end, min_win_cov = 30,
    rawdata = FALSE, max_scope = 1e6) {
    temp1 <- p4cNewProfile(
        unlist(strsplit(p4c_obj$track_nm, " ")), p4c_obj$bait$chrom,
        p4c_obj$bait$start, max_scope, max_scope
    )
    temp2 <- p4cNewProfile(
        unlist(strsplit(ref_p4c_obj$track_nm, " ")), ref_p4c_obj$bait$chrom,
    ...
    if (!rawdata) {
        ...
        slice_df <- data.frame(min(comp_list$trend_mat[scope_idx, 1]), max(comp_list$trend_mat[
            scope_idx,
            1
        ]), mean(comp_list$trend_mat[scope_idx, 2]), mean(comp_list$trend_mat[
            scope_idx,
            3
        ]), mean(comp_list$trend_mat[scope_idx, 4]))
    } else {
        scope_idx <- which(ref_p4c_obj$dgram[, 1] >= start & ref_p4c_obj$dgram[
            ,
            1
        ...
        slice_df <- data.frame(min(p4c_obj$dgram.norm$dgram[scope_idx, 1]), max(p4c_obj$dgram.norm$dgram[
            scope_idx,
            1
        ]), mean(p4c_obj$dgram.norm$dgram[scope_idx, 2]), mean(ref_p4c_obj$dgram[
            scope_idx,
            2
        ]), "NA")
    }
    ...
    colnames(slice_df) <- c(
        "start", "end", p4c_obj$track_nm, ref_p4c_obj$track_nm,
        "mean_scale"
    )
    ...
    mat <- matrix(c(n1, N1 - n1, n2, N2 - n2), nrow = 2, byrow = F, dimnames = list(c(
        "win",
        "tot"
    ), c("1", "2")))
...
.p4cGenerateBaitDgram.p4cProfile <- function(p4c_obj) {
    ...
    re_scope <- gextract(re_map_track, re_fl_track,
        intervals = cis_scope, iterator = re_map_track,
        colnames = c("map", "fl")
    )
    ...
    re_interv <- gintervals.2d(
        rep(bait_chrom, n), rep(
            as.numeric(bait_start) - bait_lookup_expansion,
            n
        ), rep(as.numeric(bait_start) + bait_lookup_expansion, n), re_map$chrom, re_map$start,
        re_map$end
    )
...
.p4cGenCovNormVector <- function(
    p4c_obj, norm_bins = 10^(3:6), r_expand = 1.2, post_smooth_win = 50,
    stat_type = "log") {
    bins <- sort(norm_bins)
    ...
    if (max(scope) > max(bins)) {
        message(sprintf(
            "Error: Max normalizing bin (%i) is smaller than max scope (%i)",
            max(bins), max(scope)
        ))
...
.p4cSmoothedTrendComp.p4cProfile <- function(
    p4c_obj1, ref_p4c_obj, min_win_cov = 30,
    min_res = 1) {
    ...
    coord[is.na(coord)] <- base_coord[is.na(coord)]
    trend_mat <- data.frame(
        start = sort(coord), p4c_obj1 = vec1.norm, ref_p4c_obj = vec2,
        scale = scale
    )
...
.p4cWinGeoMeanCoordinate <- function(coords, scale, bait_x) {
    ...
    near_bait2 <- sapply((bait_idx + 1):(bait_idx + scale / 2 - 1), function(i) {
        exp(mean(log(offsets[(bait_idx +
            1):i])))
    })
...
.p4cSetCurBait.p4cProfile <- function(
    p4c_obj, bait_chrom = NULL, bait_start = NULL,
...
.p4cSetDgramParams.p4cProfile <- function(
    p4c_obj, re_seq = getOption("TG3C.RE_seq"),
    ...
    l <- list(
        re_seq = re_seq, map_thresh = as.numeric(map_thresh), min_flen = as.numeric(min_flen),
...
.p4cSetPngParams.p4cProfile <- function(
    p4c_obj, png_w = getOption("TG3C.png_w"), png_h = getOption("TG3C.png_h"),
    png_dir = getOption("TG3C.png_dir"), png_res = getOption("TG3C.png_res")) {
    l <- list(
        png_w = as.numeric(png_w), png_h = as.numeric(png_h), png_dir = png_dir,
...
.p4cSetDgramGraphics.p4cProfile <- function(
    p4c_obj, zlim = getOption("TG3C.p4c_dgram_zlim"),
...
.image.scale <- function(
    z, zlim, col = heat.colors(12), breaks, horiz = TRUE, ylim = NULL,
    xlim = NULL, yaxt = "n", ...) {
    if (!missing(breaks)) {
    ...
    plot(1, 1,
        t = "n", ylim = ylim, xlim = xlim, xaxt = xaxt, yaxt = yaxt, xaxs = "i",
        yaxs = "i", ...
    )
    for (i in seq(poly)) {
        if (horiz) {
