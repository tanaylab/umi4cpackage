...
We will now create a track with the mappability.
...
To create a redb you need to provide a tabulation-delimited file which provide the list of chromosomes and the path to the sequence.
```{r, eval=FALSE}
seq.files <- list.files(file.path(GROOT, "seq"))
seq.names <- sapply(seq.files, function(fn){gsub("^chr", "", 
                                                 gsub("\\.seq$", "", fn))})
seq.key.df <- data.frame(names = seq.names,
                         files = file.path(GROOT, "seq", seq.files))
...
```{r, eval=FALSE, warning=FALSE}
# For example, DpnII:
gtrack.create_redb_tracks = function(re_seq="GATC", redb_params_fn="my_genome/conf/redb.conf")
```
